...
# parse complete specifications of a phone given complete url of the phone and write to database
...
if __name__ == '__main__':
    ...
    with sqlite3.connect(DATABASE_FILE) as con:
        cur = con.cursor()
        name = "Apple iPhone 6"
        query = 'insert into PhoneSpecs(Name, Specs) values("%s", ?)' % ( name )
        print query
        print ""
        
        cur.execute(query,  ( json.dumps(loaded_data), ))
        con.commit()
